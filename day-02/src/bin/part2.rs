...
fn is_safe_difference(difference: i32) -> bool {
    is_safe_difference_delta(difference, MAX_SAFE_DIFFERENCE)
}

fn is_safe_difference_delta(difference: i32, max_difference: i32) -> bool {
    difference.abs() > 0 && difference.abs() <= max_difference
}
...
fn count_safe_lines(lines: &[String]) -> i32 {
    ...
    for line in lines {
        ...
        if is_safe_or_is_safe_after_removing_one_sample(&number_vec) {
            safe_lines += 1;
        }
    }
    safe_lines
...
mod tests {
    ...
    fn test_count_safe_line_check() {
        ...
        let expect = vec![true, false, false, true, true, true];
        for (line, &expect) in lines.iter().zip(expect.iter()) {
            let number_vec = parse_line_to_number_vector(line);
            assert_eq!(
                is_safe_or_is_safe_after_removing_one_sample(&number_vec),
                expect,
                "line: {}",
                line
            );
    ...
    #[test]
    fn test_is_not_inreasing_or_decreasing() {
        let number_vec = vec![1, 3, 2, 4, 5];
        assert_eq!(is_increasing(&number_vec), false);
        assert_eq!(is_decreasing(&number_vec), false);
    }

    #[test]
    fn test_broken_line() {
        let line = "1 2 7 8 9".to_string();
        let number_vec = parse_line_to_number_vector(&line);
